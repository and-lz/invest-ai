...
export const ComparacaoRetornoSchema = z.object({
  periodo: z
    .string()
    .describe("Ex: 'No mes', '3 meses', '12 meses', 'Desde o inicio'"),
...
export const AnaliseRendaPassivaSchema = z.object({
  yieldMedioMensal: z
    .number()
    .nullable()
    .describe("Percentual medio mensal de proventos"),
  yieldAnualizado: z
    .number()
    .nullable()
    .describe("Yield anualizado (% a.a.)"),
  yieldOnCost: z
    .number()
    .nullable()
    .describe("Yield sobre o custo medio de aquisicao"),
  totalRecebidoCentavos: z
    .number()
    .int()
    .describe("Total em centavos de proventos recebidos"),
  consistencia: z
    .string()
    .describe("Avaliacao da regularidade dos pagamentos"),
  comparacaoComSelic: z
    .string()
    .nullable()
    .describe("Como se compara com a SELIC vigente"),
});
...
export const AvaliacaoFundamentalistaSchema = z.object({
  ...
  resumoAvaliacao: z
    .string()
    .describe("Interpretacao dos fundamentos em linguagem acessivel"),
  comparacaoSetorial: z
    .string()
    .nullable()
    .describe("Como se compara com pares do setor"),
});

export const AvaliacaoTimingSchema = z.object({
  resumo: z
    .string()
    .describe("Avaliacao geral das decisoes de compra/venda"),
  precoMedioEstimadoCentavos: z
    .number()
    .int()
    .nullable()
    .describe("Preco medio de entrada em centavos"),
  momentosFavoraveis: z
    .array(z.string())
    .describe("Decisoes que foram boas"),
  momentosDesfavoraveis: z
    .array(z.string())
    .describe("Decisoes que poderiam ter sido melhores"),
});
...
export const VeredictoSchema = z.object({
  ...
  justificativa: z
    .string()
    .describe("Explicacao clara do porque da recomendacao"),
  horizonteTemporal: z
    .string()
    .describe("Ex: 'Proximo 3-6 meses', 'Longo prazo (12+ meses)'"),
  condicoesRevisao: z
    .string()
    .describe("Em que condicoes a recomendacao mudaria"),
});
...
export const AnaliseAtivoResponseSchema = z.object({
  ...
  resumoGeral: z
    .string()
    .describe("2-3 frases com o veredicto principal. Direto e opinativo."),

  analisePerformance: z.object({
    comparacoes: z.array(ComparacaoRetornoSchema),
    tendenciaRecente: z
      .string()
      .describe("Descricao da tendencia nos ultimos 3 meses"),
    posicaoNaCarteira: z
      .string()
      .describe("Contexto da participacao % na carteira total"),
  }),
  ...
  pontosDeAtencao: z
    .array(z.string())
    .describe("Lista de coisas para o usuario monitorar"),
});
...
export type AvaliacaoFundamentalista = z.infer<
  typeof AvaliacaoFundamentalistaSchema
>;
