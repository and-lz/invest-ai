...
export function ProvedorContextoPaginaChat({
  children,
}: {
  children: ReactNode;
}) {
  const [identificadorPagina, setIdentificadorPagina] =
    useState<IdentificadorPagina>("dashboard");
  const [dadosContexto, setDadosContexto] = useState<string | undefined>(
    undefined,
  );

  const definirContexto = useCallback(
    (identificador: IdentificadorPagina, dados?: string) => {
  ...
  return (
    <ContextoPaginaChat.Provider value={valor}>
      {children}
    </ContextoPaginaChat.Provider>
  );
}
...
export function useContextoPaginaChat(): ContextoPaginaChatValor {
  ...
  if (!contexto) {
    throw new Error(
      "useContextoPaginaChat deve ser usado dentro de ProvedorContextoPaginaChat",
    );
  }
  return contexto;
}
