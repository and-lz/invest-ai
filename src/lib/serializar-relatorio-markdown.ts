...
function formatarDinheiroCompacto(money: Money | null): string {
  if (!money) return "N/D";
  return formatarMoedaCompacta(money.valorEmCentavos);
}
...
function serializarMetadados(metadados: RelatorioExtraido["metadados"]): string {
  ...
  const linhas = [
    ...
    `- Data de geracao: ${formatarDataBrasileira(metadados.dataGeracao)}`,
  ];
  return linhas.join("\n");
}
...
function serializarResumo(resumo: Resumo): string {
  ...
  linhas.push(
    `- Rentabilidade desde inicio: ${formatarPercent(resumo.rentabilidadeDesdeInicio)} (desde ${formatarDataBrasileira(resumo.dataInicioCarteira)})`,
  );
