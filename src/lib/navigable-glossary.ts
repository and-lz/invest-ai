import type { EntradaGlossario } from "./financial-glossary";
import * as Glossario from "./financial-glossary";
...
export interface NavigableGlossaryTerm extends EntradaGlossario {
 ...
 */
export const TODOS_TERMOS: NavigableGlossaryTerm[] = Object.entries(Glossario)
  .filter(([chave]) => chave.startsWith("GLOSSARY_"))
  .flatMap(([chave, valor]): NavigableGlossaryTerm[] => {
 ...
 */
export function buscarTermos(query: string): NavigableGlossaryTerm[] {
 ...
 */
export function agruparPorLetra(): Map<string, NavigableGlossaryTerm[]> {
  const mapa = new Map<string, NavigableGlossaryTerm[]>();
 ...
 */
export function agruparPorCategoria(): Map<string, NavigableGlossaryTerm[]> {
  const mapa = new Map<string, NavigableGlossaryTerm[]>();
  ...
  const mapaOrdenado = new Map<string, NavigableGlossaryTerm[]>();
 ...
 */
export function buscarTermoPorSlug(slug: string): NavigableGlossaryTerm | undefined {
