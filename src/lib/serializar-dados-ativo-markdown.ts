...
import { formatarMesAno, formatarDataBrasileira } from "@/lib/format-date";
...
function serializarCotacaoAtual(cotacao: CotacaoAtual | null): string {
  ...
  if (cotacao.marketCap) {
    linhas.push(`- Market Cap: ${formatarMoedaCompacta(Math.round(cotacao.marketCap * 100))}`);
  }
...
function serializarFundamentalistas(dados: DadosFundamentalistas | null): string {
  ...
  if (dados.lucroLiquidoCentavos !== null) {
    linhas.push(`- Lucro Liquido: ${formatarMoedaCompacta(dados.lucroLiquidoCentavos)}`);
  }
  if (dados.receitaLiquidaCentavos !== null) {
    linhas.push(`- Receita Liquida: ${formatarMoedaCompacta(dados.receitaLiquidaCentavos)}`);
  }
  if (dados.setor) {
...
function serializarDividendosBrapi(dividendos: DividendoHistorico[]): string {
  ...
  for (const dividendo of ultimosDividendos) {
    const dataEx = formatarDataBrasileira(dividendo.dataExDividendo);
    const dataPagamento = dividendo.dataPagamento
      ? formatarDataBrasileira(dividendo.dataPagamento)
      : "N/D";
    linhas.push(
