...
import { BreadcrumbsEducacional } from "./educational-breadcrumbs";
import { BotaoLerArtigo } from "./read-article-button";
import { cn } from "@/lib/utils";
import { typography, icon } from "@/lib/design-system";
import type { ArtigoMetadata } from "@/schemas/educational-article.schema";
import { INFORMACOES_CATEGORIAS } from "@/schemas/educational-article.schema";
    ...
    <article className={cn("mx-auto max-w-3xl space-y-8", className)}>
        ...
        <BreadcrumbsEducacional
          items={[
            ...
            {
              label: categoriaInfo?.titulo ?? metadata.categoria,
              href: `/aprender/${metadata.categoria}`,
            },
            { label: metadata.titulo },
          ]}
        />
        ...
        <header className="space-y-4">
          <div className="flex items-start gap-3">
            <h1 className={cn(typography.h1Large, "flex-1")}>{metadata.titulo}</h1>
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            >
              <GraduationCap className={cn("mr-1.5", icon.micro)} aria-hidden="true" />
              {ROTULOS_NIVEL[metadata.nivelDificuldade]}
            </Badge>

            <Badge variant="secondary">
              <Clock className={cn("mr-1.5", icon.micro)} aria-hidden="true" />
              {metadata.tempoLeituraMinutos} min de leitura
            </Badge>

            <Badge variant="secondary">
              <Calendar className={cn("mr-1.5", icon.micro)} aria-hidden="true" />
              ...
              <Link href={`/aprender/${artigoAnterior.categoria}/${artigoAnterior.slug}`}>
                <Card className="hover:border-primary/30 h-full transition-colors">
                  <CardContent className="flex items-center gap-3 p-4">
                    <ChevronLeft
                      className={cn(icon.cardTitle, "text-muted-foreground shrink-0")}
                      aria-hidden="true"
                    />
                    <div className="min-w-0">
                      <p className={cn(typography.helper, "tracking-wide uppercase")}>Anterior</p>
                ...
                <Card className="hover:border-primary/30 h-full transition-colors">
                  <CardContent className="flex items-center justify-end gap-3 p-4">
                    <div className="min-w-0 text-right">
                      <p className={cn(typography.helper, "tracking-wide uppercase")}>Próximo</p>
                      <p className="hover:text-primary mt-1 truncate text-sm font-semibold transition-colors">
        ...
        <div className="flex justify-center pt-8">
          <Link href={`/aprender/${metadata.categoria}`}>
            <Button variant="outline">
              <ChevronLeft className={cn("mr-2", icon.button)} aria-hidden="true" />
