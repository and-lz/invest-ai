...
export const ConteudoMarkdownChat = React.memo(
  ({ conteudo, fullscreen }: ConteudoMarkdownChatProps) => {
    const fs = fullscreen;
    return (
      <div className={cn("markdown-content prose max-w-none", fs ? "prose-xl" : "prose-sm")}>
        <ReactMarkdown
          ...
          components={{
            ...
            // Headings
            h1: ({ children }) => <h1 className={cn("mt-4 mb-2 font-semibold", fs ? "text-4xl" : "text-lg")}>{children}</h1>,
            h2: ({ children }) => <h2 className={cn("mt-3 mb-2 font-semibold", fs ? "text-3xl" : "text-base")}>{children}</h2>,
            h3: ({ children }) => <h3 className={cn("mt-3 mb-1 font-semibold", fs ? "text-2xl" : "text-sm")}>{children}</h3>,
          }}
        >
          {conteudo}
        </ReactMarkdown>
